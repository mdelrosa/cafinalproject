...
reg [30:0] tone;
always @(posedge clk) tone <= tone+1; //creates a tone counter

wire [7:0] fullnote; 
music_rom rom(.inclock(clk), .outclock(clk), .address(tone[29:22]), .q(fullnote));
//uses a vender tool generated by the FPGA
...
// uses the divby12 function to  et the octave and note

reg [8:0] clkdivider;
always @(note)
//decides how much you need to divde the clock by to get the correct frequency for each note
case(note)
...
//takes a 6 bit number, divides it by 12 and then returns a 3 bit quotient and 4 bit remainder
input [5:0] numer;
output [2:0] quotient;
output [3:0] remain;
